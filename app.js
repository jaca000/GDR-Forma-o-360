...
function training(){if(!trainingDraft){return `<div class="section-title"><h3>Novo treino</h3></div><div class="card"><div class="field"><label>Escalão</label><select id="tgroup"><option>Benjamins</option><option>Traquinas</option><option>Todos</option></select></div><div class="express-tip">⚡ Data e hora automáticas. Todos começam como <b>Presentes</b> com avaliação <b>4 / 4 / 4</b>. Só tens de alterar as exceções.</div><button class="btn btn-primary btn-block" onclick="startTraining()">⚡ Iniciar Registo Express</button></div>`}
...
function startTraining(){const n=new Date();const date=n.getFullYear()+'-'+String(n.getMonth()+1).padStart(2,'0')+'-'+String(n.getDate()).padStart(2,'0');const time=String(n.getHours()).padStart(2,'0')+':'+String(n.getMinutes()).padStart(2,'0');trainingDraft={id:uid('t'),date,time,group:el('tgroup').value,createdBy:session.userId,records:{}};db.athletes.filter(a=>a.active&&(trainingDraft.group==='Todos'||a.group===trainingDraft.group||a.group==='Traquinas/Benjamins')).forEach(a=>ensureRec(a.id));render()}
...
function cardTraining(a){const r=ensureRec(a.id);return `<div class="card"><div class="athlete"><div class="avatar">${initials(a.name)}</div><div class="grow"><strong>${esc(a.name)}</strong><div class="muted">${esc(a.group)}</div></div></div><div class="attendance">${['Presente','Falta','Justificada'].map(s=>`<button class="${r.status===s?'sel-'+(s==='Presente'?'present':s==='Falta'?'absent':'justified'):''}" onclick="setStatus('${a.id}','${s}')">${s==='Presente'?'✅':s==='Falta'?'❌':'🟡'} ${s}</button>`).join('')}</div>${r.status==='Presente'?`<div class=\"quick-presets\"><button onclick=\"preset('${a.id}','excellent')\">🔥<span>Excelente</span></button><button onclick=\"preset('${a.id}','good')\">👍<span>Bom</span></button><button onclick=\"preset('${a.id}','attention')\">⚠️<span>Atenção</span></button><button onclick=\"preset('${a.id}','behavior')\">🚨<span>Comport.</span></button></div>`:''}${r.status==='Presente'?['attitude|Atitude','effort|Empenho','behavior|Comportamento'].map(z=>{const[k,l]=z.split('|');return `<div class="metric"><div class="metric-head"><b>${l}</b><span>${r[k]}/5</span></div><div class="rating">${[1,2,3,4,5].map(n=>`<button class="${Number(r[k])===n?'active':''}" onclick="rate('${a.id}','${k}',${n})">${n}</button>`).join('')}</div></div>`}).join(''):''}<div class="field"><textarea rows="2" placeholder="Observação opcional" onchange="note('${a.id}',this.value)">${esc(r.note||'')}</textarea></div></div>`}
function preset(id,type){const r=ensureRec(id);r.status='Presente';if(type==='excellent')Object.assign(r,{attitude:5,effort:5,behavior:5});if(type==='good')Object.assign(r,{attitude:4,effort:4,behavior:4});if(type==='attention')Object.assign(r,{attitude:3,effort:3,behavior:3});if(type==='behavior')Object.assign(r,{behavior:2});render()}
